...
def getPrediction(arr_in): 
    features = ['Games Played', 'Games Started', 'MPG', 'FG', 'FGA', '3P', '3PA', '2P', '2PA',
                'FT', 'FTA', 'ORB', 'DRB', 'TRB', 'AST', 'STL', 'BLK', 'PTS', 'TOV',
                'Previous Times All-Star', 'Win Percent', 'PTS/FGA']
    ...
    if (float(in_df['MPG'][0]) < 10):
        # predictions get weird for players under 10 mins per game, just say this is a no
        prob[1] = 0
...
def get_history():
    f = open(os.path.join(basedir, "static/data/updatelog.txt"), 'r')
    timelog = f.readline()
    return render_template('history.html', timelog=timelog)
